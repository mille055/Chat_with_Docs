...
def get_text(pdf_files):
    ...
    for pdf_file in pdf_files:
        ...
        for index, page in enumerate(pdf_reader.pages):
            ...
            current_page_text = current_page_text.replace('\u2009', ' ').replace('\xa0', ' ')
            current_page_text = ' '.join(current_page_text.split())

            text_dict[(pdf_file, index)] = current_page_text 
    ...
    with st.sidebar:
        ...
        if st.button("Process Files"):
            ...
            with st.spinner("Processing"):
                ...
                raw_text, text_dict = get_text(pdf_files)
                print(raw_text)

                # Get the chunks of text
                text_chunks = chunk_text_with_overlap(text_dict, max_chunk_length, chunk_overlap)
                print(text_chunks)

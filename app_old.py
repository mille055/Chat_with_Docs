...
def run_UI():
    ...
    for i, (user_text, rag_resp, llm_resp, source_pages) in enumerate(zip(
        st.session_state['user_queries'],
        st.session_state['rag_conversation'],
        st.session_state['llm_conversation'],
        st.session_state['source_pages'])):
    # for i in range(len(st.session_state['user_queries'])):
    #     user_text = st.session_state['user_queries'][i]
    #     rag_response = st.session_state['rag_conversation'][i] if i < len(st.session_state['rag_conversation']) else ""
    #     llm_response = st.session_state['llm_conversation'][i] if i < len(st.session_state['llm_conversation']) else ""
    #     source_pages = st.session_state['source_pages'][i] if i < len(st.session_state['source_pages']) else []
        
        print(user_text, rag_resp, llm_resp, source_pages)
        st.text_area(f"Question {i+1}", value=user_text, height=75, disabled=True)

        col1, col2 = st.columns(2)
        with col1:
            print(f'Rag response {i+1}: ',rag_response )
            st.text_area(f"RAG Response {i+1}", value=rag_resp, height=250, disabled=True)
        with col2:
            st.text_area(f"LLM Response {i+1}", value=llm_resp, height=250, disabled=True)
    ...
    new_query = st.text_input("Ask a question", key='new_query', value='')
    ...
    if submit_button_pressed and st.session_state['new_query']:
        # concatenate the new query with the conversation
        rag_query = '\n'.join(st.session_state['rag_conversation']) + '\nQ: ' + new_query
        llm_query = '\n'.join(st.session_state['llm_conversation']) + '\nQ: ' + new_query
    
        response = 'A:' + st_rag.generate_response(rag_query)
        llm_response = 'A: '+ st_rag.integrate_llm(llm_query)
        ...
        # update rag conversation with Q and A
        st.session_state['rag_conversation'].append('Q: '+ new_query+'\n'+response)

        # update LLM conversation with q and A
        st.session_state['llm_conversation'].append('Q: '+ new_query+'\n'+llm_response)

        print(st.session_state['rag_conversation'])
